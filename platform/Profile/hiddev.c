...
void HidDev_Init() {
    ...
    {
#if VP_BLE_BRINGUP_DISABLE_AUTO_SYNC_WL
...
uint8_t HidDev_IsSecureConnected(void) {
    return (hidDevGapState == GAPROLE_CONNECTED && hidDevConnSecure) ? TRUE
                                                                     : FALSE;
}

uint8_t HidDev_IsReportNotifyEnabled(uint8_t id, uint8_t type) {
    hidRptMap_t*     pRpt;
    gattAttribute_t* pAttr;
    uint16_t         retHandle;

    if (hidDevGapState != GAPROLE_CONNECTED || !hidDevConnSecure) {
        return FALSE;
    }
...
static void hidDevPairStateCB(uint16_t connHandle, uint8_t state,
    ...
    if (state == GAPBOND_PAIRING_STATE_COMPLETE) {
        if (status == SUCCESS) {
            ...
            if (HidDev_IsReportNotifyEnabled(HID_RPT_ID_MOUSE_IN,
...
static void hidDevRefreshSecurityState(uint16_t    connHandle,
                                       const char* reason) {
