...
bStatus_t Batt_AddService(void) {
    ...
    status =
        GATTServApp_RegisterService(battAttrTbl, GATT_NUM_ATTRS(battAttrTbl),
    ...
    switch (uuid) {
        case GATT_CLIENT_CHAR_CFG_UUID:
            status = GATTServApp_ProcessCCCWriteReq(
                connHandle, pAttr, pValue, len, offset, GATT_CLIENT_CFG_NOTIFY);
            if (status == SUCCESS) {
...
static uint8_t battMeasure(void) {
    ...
    if (adc >= battMaxLevel) {
        ...
        if (battServiceCalcCB != NULL) {
            ...
            percent =
                (uint8_t)((((adc - battMinLevel) * 25) + (range - 1)) / range);
        }
    }
