...
void Lib_Write_Flash_584X(uint32_t addr, uint32_t num, uint32_t *pBuf) {
    ...
    EEPROM_ERASE(addr & 0xFFFFF000,
                 ((BLE_SNV_BLOCK * BLE_SNV_NUM + EEPROM_BLOCK_SIZE - 1) /
                  EEPROM_BLOCK_SIZE) *
                     EEPROM_BLOCK_SIZE);
...
void CH58x_BLEInit(void) {
    ...
    cfg.tsCB =
        HAL_GetInterTempValue;  // 根据温度变化校准RF和内部RC( 大于7摄氏度 )
#if (CLK_OSC32K)
...
tmosEvents HAL_ProcessEvent(tmosTaskID task_id, tmosEvents events) {
    ...
    if (events & HAL_REG_INIT_EVENT) {
        uint8_t x32Kpw;
#if (defined BLE_CALIBRATION_ENABLE) && \
    (BLE_CALIBRATION_ENABLE == TRUE)  // 校准任务，单次校准耗时小于10ms
#ifndef RF_8K
