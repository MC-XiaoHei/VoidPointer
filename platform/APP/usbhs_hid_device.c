...
static const uint8_t g_usbhs_hid_device_desc[] = {
    ...
    0x00,
    0x01,
    USBHS_HID_STRING_MANUFACTURER,
    USBHS_HID_STRING_PRODUCT,
    USBHS_HID_STRING_SERIAL,
    0x01,
};
...
static const uint8_t g_usbhs_hid_config_desc[] = {
    ...
    0x07,
    0x05,
    USBHS_HID_MOUSE_EP_IN_ADDR,
    0x03,
    USBHS_HID_MOUSE_EP_SIZE,
    0x00,
    0x01,

    0x09,
    0x04,
    USBHS_HID_VENDOR_INTERFACE,
    0x00,
    0x02,
    0x03,
    0x00,
...
static const uint8_t g_usbhs_hid_vendor_report_desc[] = {
    0x06, 0x00, 0xFF, 0x09, 0x01, 0xA1, 0x01, 0x09, 0x02, 0x15, 0x00, 0x26,
    0xFF, 0x00, 0x75, 0x08, 0x95, 0x40, 0x81, 0x02, 0x09, 0x02, 0x15, 0x00,
...
static const uint8_t g_usbhs_hid_product_desc[] = {
    0x22, 0x03, 'V', 0, 'o', 0, 'i', 0, 'd', 0, ' ', 0, 'P', 0, 'o', 0, 'i', 0,
    'n',  0,    't', 0, 'e', 0, 'r', 0, ' ', 0, 'C', 0, 'o', 0, 'm', 0,
};
...
__attribute__((
    aligned(4))) static uint8_t g_ep1_tx_buf[USBHS_HID_MOUSE_EP_SIZE];
__attribute__((
    aligned(4))) static uint8_t g_ep2_rx_buf[USBHS_HID_VENDOR_EP_SIZE];
__attribute__((
    aligned(4))) static uint8_t g_ep3_tx_buf[USBHS_HID_VENDOR_EP_SIZE];
...
static void usbhs_handle_get_descriptor(uint16_t value, uint16_t index,
                                        uint16_t req_len, uint8_t* out_err) {
    ...
    const uint16_t first_len = g_ep0_desc_remaining > USBHS_HID_EP0_SIZE
                                   ? USBHS_HID_EP0_SIZE
                                   : g_ep0_desc_remaining;
    if (first_len != 0u) {
    ...
    if ((req->bmRequestType & USB_REQ_TYP_MASK) == USB_REQ_TYP_STANDARD) {
        switch (req->bRequest) {
            case USB_GET_DESCRIPTOR:
                usbhs_handle_get_descriptor(req->wValue, req->wIndex,
                                            req->wLength, &err);
                break;
            ...
            case USB_SET_INTERFACE:
                ...
                break;
            case USB_GET_STATUS:
                g_ep0_buf[0] = (g_dev_sleep_status & USBHS_HID_REMOTE_WAKEUP)
                                   ? 0x02u
                                   : 0x00u;
                g_ep0_buf[1] = 0u;
            ...
            case USB_CLEAR_FEATURE:
                if ((req->bmRequestType & USB_REQ_RECIP_MASK) ==
                        USB_REQ_RECIP_DEVICE &&
                    (uint8_t)(req->wValue & 0xFFu) ==
        ...
        if (!usbhs_interface_valid(interface_index)) {
            ...
            switch (req->bRequest) {
                ...
                case USBHS_HID_REQ_SET_PROTOCOL:
                    g_hid_protocol[interface_index] =
                        (uint8_t)(req->wValue & 0xFFu);
                    reply_len = 0u;
                    R16_U2EP0_T_LEN = reply_len;
                    break;
                case USBHS_HID_REQ_GET_REPORT:
                    ...
                    reply_len = req->wLength > USBHS_HID_EP0_SIZE
                                    ? USBHS_HID_EP0_SIZE
                                    : req->wLength;
                    R16_U2EP0_T_LEN = reply_len;
                    break;
...
void USBHS_HidDevice_Init(void) {
    ...
    R16_CLK_SYS_CFG |=
        (RB_CLK_SYS_MOD & 0x40) | RB_XROM_SCLK_SEL | RB_OSC32M_SEL;
...
uint8_t USBHS_HidDevice_SendMouseReport(const uint8_t* report, uint16_t len) {
    ...
    R16_U2EP1_T_LEN = len;
    R8_U2EP1_TX_CTRL = (R8_U2EP1_TX_CTRL & (uint8_t)~USBHS_UEP_T_RES_MASK) |
                       USBHS_UEP_T_RES_ACK;
    return 1u;
}
...
uint8_t USBHS_HidDevice_SendVendorReport(const uint8_t* report, uint16_t len) {
    ...
    R16_U2EP3_T_LEN = len;
    R8_U2EP3_TX_CTRL = (R8_U2EP3_TX_CTRL & (uint8_t)~USBHS_UEP_T_RES_MASK) |
                       USBHS_UEP_T_RES_ACK;
    return 1u;
}
...
void USB2_DEVICE_IRQHandler(void) {
    ...
    if (intflag & USBHS_UDIF_TRANSFER) {
        ...
        if ((intst & USBHS_UDIS_EP_DIR) == 0u) {
            if (endp_num == DEF_UEP0 &&
                (R8_U2EP0_RX_CTRL & USBHS_UEP_R_SETUP_IS) != 0u) {
                usbhs_handle_setup();
                ...
                if ((R8_U2EP2_RX_CTRL & USBHS_UEP_R_TOG_MATCH) != 0u) {
                    const uint16_t rx_len =
                        R16_U2EP2_RX_LEN > USBHS_HID_VENDOR_EP_SIZE
                            ? USBHS_HID_VENDOR_EP_SIZE
                            : R16_U2EP2_RX_LEN;
                    vp_on_vendor_report_rx(VP_HID_ROUTE_USB, g_ep2_rx_buf,
                                           rx_len, c_vp_rtc_millis());
                    R8_U2EP2_RX_CTRL ^= USBHS_UEP_R_TOG_DATA1;
                    R8_U2EP2_RX_CTRL =
                        (R8_U2EP2_RX_CTRL & (uint8_t)~USBHS_UEP_R_RES_MASK) |
                        USBHS_UEP_R_RES_ACK;
                }
            ...
            switch (endp_num) {
                case DEF_UEP0:
                    if (g_ep0_desc_remaining != 0u && g_ep0_desc_ptr != NULL) {
                        const uint16_t tx_len =
                            g_ep0_desc_remaining > USBHS_HID_EP0_SIZE
                                ? USBHS_HID_EP0_SIZE
                                : g_ep0_desc_remaining;
                        ...
                        R8_U2EP0_TX_CTRL = (R8_U2EP0_TX_CTRL &
                ...
                case DEF_UEP1:
                    R8_U2EP1_TX_CTRL =
                        (R8_U2EP1_TX_CTRL & (uint8_t)~USBHS_UEP_T_RES_MASK) |
                        USBHS_UEP_T_RES_NAK;
                ...
                case DEF_UEP3:
                    R8_U2EP3_TX_CTRL =
                        (R8_U2EP3_TX_CTRL & (uint8_t)~USBHS_UEP_T_RES_MASK) |
                        USBHS_UEP_T_RES_NAK;
