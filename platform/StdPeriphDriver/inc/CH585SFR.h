...
#define R8_SAFE_ACCESS_SIG \
    (*((PUINT8V)0x40001040))  // WO, safe accessing sign register, must write SAFE_ACCESS_SIG1 then SAFE_ACCESS_SIG2 to enter safe accessing mode
#define RB_SAFE_ACC_MODE \
    0x03  // RO, current safe accessing mode: 11=safe/unlocked (SAM), other=locked (00..01..10..11)
#define RB_CHIP_ID0 0x04  // RO, chip id0
#define RB_SAFE_ACC_ACT \
    0x08  // RO, indicate safe accessing status now: 0=locked, read only, 1=safe/unlocked (SAM), write enabled
...
#define R8_WDOG_COUNT \
    (*((PUINT8V)0x40001043))  // RW, watch-dog count, count by clock frequency Fsys/131072
...
#define RB_ROM_CODE_OFS \
    0x10  // RWA, code offset address selection in Flash ROM: 0=start address 0x000000, 1=start address 0x040000
#define RB_ROM_CTRL_EN \
    0x20  // RWA, enable flash ROM control interface enable: 0=disable access, 1=enable access control register
#define RB_ROM_DATA_WE \
    0x40  // RWA, enable flash ROM data & code area being erase/write: 0=all writing protect, 1=enable data area program and erase
#define RB_ROM_CODE_WE \
    0x80  // RWA, enable flash ROM code area being erase/write: 0=code writing protect, 1=enable code area program and erase
#define R8_GLOB_CFG_INFO \
    (*((PUINT8V)0x40001045))  // RO, global configuration information and status
#define RB_CFG_ROM_READ \
    0x01  // RO, indicate protected status of Flash ROM code and data: 0=reading protect, 1=enable read by external programmer
...
#define RB_BOOT_LOADER \
    0x20  // RO, indicate boot loader status: 0=application status (by software reset), 1=boot loader status
...
#define RB_WDOG_RST_EN \
    0x02  // RWA, enable watch-dog reset if watch-dog timer overflow: 0=as timer only, 1=enable reset if timer overflow
#define RB_WDOG_INT_EN \
    0x04  // RWA, watch-dog timer overflow interrupt enable: 0=disable, 1=enable
#define RB_WDOG_INT_FLAG \
    0x10  // RW1, watch-dog timer overflow interrupt flag, cleared by RW1 or reload watch-dog count or __SEV(Send-Event)
...
#define RB_CLK_SYS_MOD \
    0xC0  // RWA, system clock source mode: 00=divided from 16MHz/32MHz, 01=divided from PLL-624MHz, 10=directly from 16MHz/32MHz, 11=directly from 32KHz
...
#define RB_PLL_GATE_TIME \
    0x2000  // RWA, PLL clk time sel closed when source clk changed

#define R8_HFCK_PWR_CTRL \
    (*((PUINT8V)0x4000100A))  // RWA, power control of system high frequeny clock , SAM
...
#define RB_GPIO_EDGE_WAKE \
    0x04  // RWA, enable GPIO edge waking,no mater rising or falling
...
#define RB_WAKE_EV_MODE \
    0x40  // RWA, event wakeup mode: 0=event keep valid for long time, 1=short pulse event
...
#define RB_RAM_RETENT_LV \
    0xC0  // RWA, RAM low voltage retention enable: 00=forbiden, 01=LPM_1, 10=LPM_2, 11=LPM_3,

#define R32_SAFE_MODE_CTRL \
    (*((PUINT32V)0x40001010))  // RWA, safe mode control register
#define R8_SAFE_MODE_CTRL \
    (*((PUINT8V)0x40001010))  // RW, safe mode control register
#define RB_SAFE_AUTO_EN  0x01  // RW, disable safe mode auto close
#define RB_XROM_312M_SEL 0x10  // RW, XROM CLK 312MHz sel
#define R8_SAFE_CLK_CTRL \
    (*((PUINT8V)0x40001011))  // RWA, main clk off control register
...
#define R8_SAFE_DEBUG_CTRL \
    (*((PUINT8V)0x40001012))  // RWA, DEBUG control register
#define RB_DEBUG_DIS 0x10  // RWA, disable DEBUG
#define R8_SAFE_LRST_CTRL \
    (*((PUINT8V)0x40001013))  // RWA, long reset control register
...
#define RB_PIN_TMR0 \
    0x01  // RW, TMR0 alternate pin enable: 0=TMR0/PWM0/CAP0 on PA[9], 1=TMR0_/PWM0_/CAP0_ on PB[23]
#define RB_PIN_TMR1 \
    0x02  // RW, TMR1 alternate pin enable: 0=TMR1/PWM1/CAP1 on PA[10], 1=TMR1_/PWM1_/CAP1_ on PB[10]
#define RB_PIN_TMR2 \
    0x04  // RW, TMR2 alternate pin enable: 0=TMR2/PWM2/CAP2 on PA[11], 1=TMR2_/PWM2_/CAP2_ on PB[11]
#define RB_PIN_TMR3 \
    0x08  // RW, TMR3 alternate pin enable: 0=TMR3/PWM3/CAP3 on PA[2], 1=TMR3_/PWM3_/CAP3_ on PB[22]
#define RB_PIN_UART0 \
    0x10  // RW, RXD0/TXD0 alternate pin enable: 0=RXD0/TXD0 on PB[4]/PB[7], 1=RXD0_/TXD0_ on PA[15]/PA[14]
#define RB_PIN_UART1 \
    0x20  // RW, RXD1/TXD1 alternate pin enable: 0=RXD1/TXD1 on PA[8]/PA[9], 1=RXD1_/TXD1_ on PB[12]/PB[13]
#define RB_PIN_UART2 \
    0x40  // RW, RXD2/TXD2 alternate pin enable: 0=RXD2/TXD2 on PA[6]/PA[7], 1=RXD2_/TXD2_ on PB[22]/PB[23]
#define RB_PIN_UART3 \
    0x80  // RW, RXD3/TXD3 alternate pin enable: 0=RXD3/TXD3 on PA[4]/PA[5], 1=RXD3_/TXD3_ on PB[20]/PB[21]
#define RB_PIN_SPI0 \
    0x0100  // RW, SCS/SCK0/MOSI/MISO alternate pin enable: 0=SCS/SCK0/MOSI/MISO on PA[12]/PA[13]/PA[14]/PA[15], 1=SCS_/SCK0_/MOSI_/MISO_ on PB[12]/PB[13]/PB[14]/PB[15]
#define RB_PIN_PWMX \
    0x0400  // RW, PWM4/PWM5/PWM7/PWM8/PWM9 alternate pin enable: 0=PWM4/5/7/8/9 on PA[12]/PA[13]/PB[4]/PB[6]/PB[7], 1=PWM4/5/7/8/9 on PA[6]/PA[7]/PB[1]/PB[2]/P[3]
#define RB_PIN_I2C \
    0x0800  // RW, SCL/SDA alternate pin enable: 0=SCL/SDA on PB[13]/PB[12], 1=SCL_/SDA_ on PB[21]/PB[20]
#define RB_PIN_MODEM \
    0x1000  // RW, DSR/DTR alternate pin enable: 0=DSR/DTR on PB[1]/PB[5], 1=DSR_/DTR_ on PB[14]/PB[15]
#define RB_PIN_INTX \
    0x2000  // RW, INT24/25 alternate pin enable: 0=INT24/25 on PB[8]/PB[9], 1=INT24/25 on PB[22]/PB[23]
#define RB_PIN_U0_INV 0x4000  // RW, UART0 in/out inverting enable
#define RB_RF_ANT_SW_EN \
    0x8000  // RW, RF antenna switch control output enable: 0=disable output, 1=output on PB[16]/PB[17]/PB[18]/PB[19]/PB[20]/PB[21]
#define R16_PIN_CONFIG \
    (*((PUINT16V)0x4000101A))  // RW, functional pin enable and digital input disable
#define RB_PB16_8_SEL 0x10  // RW, interupt pin select,0=PB8, 1=PB16
#define RB_PIN_USB2_EN \
    0x20  // RW, USB high speed comunication pin select, 0=PB12/13 not use, 1=PB12/13 used
#define RB_UDP_PU_EN \
    0x40  // RW, enable USB ud+ pin pulled upresistance , 0=control by RB_UC_DEV_PU_EN, 1=force pulled up
#define RB_PIN_USB_EN \
    0x80  // RW, enable USB fast speed pin, 0=PB10/PB11 not used, 1=PB10/PB11 used
...
#define RB_PWR_DCDC_EN \
    0x0200  // RWA, DC/DC converter enable: 0=DC/DC disable and bypass, 1=DC/DC enable
...
#define RB_XT_PRE_EN \
    0x4000  // RWA, extern 32MHz HSE early wake up enable, must be used with LSI/LSE
...
#define RB_XT32K_I_TUNE \
    0x03  // RWA, external 32KHz oscillator current tune: 00=70% current, 01=standard current, 10=140% current, 11=200% current for startup
#define RB_RC32K_I_TUNE \
    0x0C  // RWA, internal 32KHz oscillator current tune: 00=70% current, 01=standard current, 10=140% current, 11=200% current for startup
#define RB_XT32K_C_LOAD \
    0xF0  // RWA, external 32KHz oscillator load capacitor tune: Cap = RB_XT32K_C_LOAD + 12pF
...
#define RB_CLK_OSC32K_FILT \
    0x08  // RWA, internal 32KHz oscillator low noise mode disable: 1=enable, 0=disable
...
#define RB_RTC_TMR_MODE \
    0x07  // RWA, RTC timer mode: 000=0.125S, 001=0.25S, 010=0.5S, 011=1S, 100=2S, 101=4S, 110=8S, 111=16S
...
#define R16_RTC_CNT_2S \
    (*((PUINT16V)0x4000103A))  // RO, RTC count based 2 second
...
#define RB_XT32M_I_BIAS \
    0x03  // RWA, external 32MHz oscillator bias current tune: 00=75% current, 01=standard current, 10=125% current, 11=150% current
#define RB_XT32M_C_LOAD \
    0xF0  // RWA, external 32MHz oscillator load capacitor tune: if bit7 = 0,Cap = RB_XT32M_C_LOAD * 2 + 10pF,else if bit7 = 1,Cap = RB_XT32M_C_LOAD * 2 + 2pF,
...
#define R16_OSC_CAL_CNT \
    (*((PUINT16V)0x40001050))  // RO, system clock count value for 32KHz multi-cycles
#define RB_OSC_CAL_CNT \
    0x3FFF  // RO, system clock count value for 32KHz multi-cycles
#define RB_OSC_CAL_OV_CLR \
    0x4000  // RW1, indicate R8_OSC_CAL_OV_CNT not zero, set 1 to clear R8_OSC_CAL_OV_CNT
#define RB_OSC_CAL_IF \
    0x8000  // RW1, interrupt flag for oscillator capture end, set 1 to clear
#define R8_OSC_CAL_OV_CNT \
    (*((PUINT8V)0x40001052))  // RO, oscillator frequency calibration overflow times
...
#define RB_OSC_CNT_HALT \
    0x08  // RO, calibration counter halt status: 0=counting, 1=halt for reading count value
...
#define RB_ADC_CH_INX \
    0x1F  // RW, ADC input channel index, 00~0D=A0~13, 0E=VBAT, 0F=TS, 10=NFC energy check channel
...
#define RB_ADC_CLK_DIV \
    0xC0  // RW, select ADC clock frequency: 00=16/8MHz, 01=8/4MHz, 10=5.33/2.67MHz, 11=4/2MHz, depends on R16_CLK_SYS_CFG[9] = 1/0
#define R8_ADC_CONVERT (*((PUINT8V)0x4000105A))  // RW, ADC convert control
#define RB_ADC_START \
    0x01  // RW, ADC convert start control: 0=stop ADC convert, 1=start an ADC convert, auto clear
#define RB_ADC_PGA_GAIN2 \
    0x02  // RW, ADC gain direction, must be 0 when using TS
#define RB_ADC_SAMPLE_TIME \
    0x30  // RW, ADC sample time delay,Tadc = Tsys/clk_div, 00=4*Tadc, 01=5*Tadc, 10=6*Tadc, 11=7*Tadc
#define RB_ADC_EOC_X 0x80  // RO, end of ADC conversion flag
#define R8_TEM_SENSOR \
    (*((PUINT8V)0x4000105B))  // RW, temperature sensor control
...
#define RB_ADC_IF_EOC \
    0x80  // RO, ADC conversion interrupt flag: 0=free or converting, 1=end of conversion, interrupt action, auto ADC or write R8_ADC_CONVERT or write R8_TKEY_CONVERT to clear flag
#define R32_TKEY_CTRL (*((PUINT32V)0x40001054))  // RW, Touchkey control
#define R8_TKEY_COUNT \
    (*((PUINT8V)0x40001054))  // RW, touch-key charge and discharge time register
#define RB_TKEY_DISCH_CNT \
    0xE0  // RW, number of touch-key discharge cycles, calculation method:  discharge time=(RB_TKEY_DISCH_CNT+1)*Tadc
#define RB_TKEY_CHARG_CNT \
    0x1F  // RW, number of touch-key charge cycles, calculation method:  charge time=(RB_TKEY_CHARG_CNT+3)*Tadc
#define R8_TKEY_CONVERT \
    (*((PUINT8V)0x40001056))  // RW, touch-key detection control register
#define RB_TKEY_START \
    0x01  // RW, touch-key detect control and status, 0=stop the conversion, 1=start detecting or conversion
#define R8_TKEY_CFG \
    (*((PUINT8V)0x40001057))  // RW, touch-key configure register
...
#define RB_TKEY_PGA_ADJ \
    0x08  // RW, select work speed of PGA which in ADC, 1=high speed, 0=nomarl speed
#define RB_TKEY_DRV_EN 0x04  // RW, enable touch-key drive shielding
#define RB_TKEY_CURRENT \
    0x02  // RW, select touch-key charge current, 1=60%, 0=current rating
#define RB_TKEY_PWR_ON 0x01  // RW, enable touch-key power control
#define R32_TKEY_SEL   (*((PUINT32V)0x4000107C))  // RW, Touchkey sel
#define RB_TKEY_DRV_OUTEN \
    0xFFFC0000  // RW, Touchkey multi_drive shields each channel,high enable
...
#define RB_ADC_IF_END_ADC \
    0x10  // RW1, interrupt flag for end of ADC conversion, DMA for auto ADC or write R8_ADC_CONVERT to clear flag
#define R8_ADC_AUTO_CYCLE \
    (*((PUINT8V)0x40001063))  // RW, auto ADC cycle value, unit is 16 Fsys
#define R32_ADC_DMA_NOW \
    (*((PUINT32V)0x40001064))  // RW, ADC DMA current address
...
#define R32_ADC_SCAN_CFG1 \
    (*((PUINT32V)0x40001070))  // RW, ADC channel scan config
...
#define R32_ADC_SCAN_CFG2 \
    (*((PUINT32V)0x40001074))  // RW, ADC channel scan config
...
#define RB_ADC_SCAN_CH13 \
    0x000F0000  // RW, channel13 analog input channel select
...
#define RB_ADC_SCAN_MASK_DIS \
    0x40000000  // RW, disble ADC channel when sample end
...
#define R16_PA_INT_MODE \
    (*((PUINT16V)0x40001094))  // RW, GPIO PA interrupt mode: 0=level action, 1=edge action
...
#define R8_PA_DIR_0 \
    (*((PUINT8V)0x400010A0))  // RW, GPIO PA I/O direction byte 0
#define R8_PA_DIR_1 \
    (*((PUINT8V)0x400010A1))  // RW, GPIO PA I/O direction byte 1
...
#define R32_PA_PD_DRV \
    (*((PUINT32V)0x400010B4))  // RW, PA pulldown for input or PA driving capability for output
#define R8_PA_PD_DRV_0 \
    (*((PUINT8V)0x400010B4))  // RW, PA pulldown for input or PA driving capability for output byte 0
#define R8_PA_PD_DRV_1 \
    (*((PUINT8V)0x400010B5))  // RW, PA pulldown for input or PA driving capability for output byte 1
#define R32_PA_SET \
    (*((PUINT32V)0x400010B8))  // WZ, GPIO PA set output: 0=keep, 1=set
#define R8_PA_SET_0 \
    (*((PUINT8V)0x400010B8))  // WZ, GPIO PA_0~7 set output: 0=keep, 1=set
#define R8_PA_SET_1 \
    (*((PUINT8V)0x400010B9))  // WZ, GPIO PA_8~15 set output: 0=keep, 1=set
/* GPIO PB register */
#define R32_PB_DIR \
    (*((PUINT32V)0x400010C0))  // RW, GPIO PB I/O direction: 0=in, 1=out
#define R8_PB_DIR_0 \
    (*((PUINT8V)0x400010C0))  // RW, GPIO PB I/O direction byte 0
#define R8_PB_DIR_1 \
    (*((PUINT8V)0x400010C1))  // RW, GPIO PB I/O direction byte 1
#define R8_PB_DIR_2 \
    (*((PUINT8V)0x400010C2))  // RW, GPIO PB I/O direction byte 2
...
#define R32_PB_PD_DRV \
    (*((PUINT32V)0x400010D4))  // RW, PB pulldown for input or PB driving capability for output
#define R8_PB_PD_DRV_0 \
    (*((PUINT8V)0x400010D4))  // RW, PB pulldown for input or PB driving capability for output byte 0
#define R8_PB_PD_DRV_1 \
    (*((PUINT8V)0x400010D5))  // RW, PB pulldown for input or PB driving capability for output byte 1
#define R8_PB_PD_DRV_2 \
    (*((PUINT8V)0x400010D6))  // RW, PB pulldown for input or PB driving capability for output byte 2
#define R32_PB_SET \
    (*((PUINT32V)0x400010D8))  // WZ, GPIO PB set output: 0=keep, 1=set
#define R8_PB_SET_0 \
    (*((PUINT8V)0x400010D8))  // WZ, GPIO PB_0~7 set output: 0=keep, 1=set
#define R8_PB_SET_1 \
    (*((PUINT8V)0x400010D9))  // WZ, GPIO PB_8~15 set output: 0=keep, 1=set
#define R8_PB_SET_2 \
    (*((PUINT8V)0x400010DA))  // WZ, GPIO PB_16~23 set output: 0=keep, 1=set
...
#define R32_TMR0_DMA_NOW \
    (*((PUINT32V)0x40002014))  // RW, TMR0 DMA current address
#define R32_TMR0_DMA_BEG \
    (*((PUINT32V)0x40002018))  // RW, TMR0 DMA begin address
...
#define R32_TMR1_DMA_BEG \
    (*((PUINT32V)0x40002418))  // RW, TMR1 DMA begin address
...
#define R32_TMR2_DMA_BEG \
    (*((PUINT32V)0x40002818))  // RW, TMR2 DMA begin address
...
#define R32_TMR3_DMA_NOW \
    (*((PUINT32V)0x40002C14))  // RW, TMR3 DMA current address
#define R32_TMR3_DMA_BEG \
    (*((PUINT32V)0x40002C18))  // RW, TMR3 DMA begin address
...
#define RB_TMR_OUT_POLAR \
    0x10  // RW, timer PWM output polarity: 0=default low and high action, 1=default high and low action
#define RB_TMR_CAP_COUNT \
    0x10  // RW, count sub-mode if RB_TMR_MODE_IN=1: 0=capture, 1=count
#define RB_TMR_FREQ_13_56 \
    0x20  // RW, count with 13.56Mhz only s_clk >=42Mhz,only tmr0 and tmr3 new add_on
#define RB_TMR_PWM_REPEAT \
    0xC0  // RW, timer PWM repeat mode: 00=1, 01=4, 10=8, 11-16
#define RB_TMR_CAP_EDGE \
    0xC0  // RW, timer capture edge mode: 00=disable, 01=edge change, 10=fall to fall, 11-rise to rise
...
#define RB_TMR_IE_FIFO_HF \
    0x04  // RW, enable interrupt for timer FIFO half (capture fifo >=4 or PWM fifo <=3)
...
#define RB_TMR_IF_FIFO_HF \
    0x04  // RW1, interrupt flag for timer FIFO half (capture fifo >=4 or PWM fifo <=3)
...
#define R8_UART0_DIV \
    (*((PUINT8V)0x4000300E))  // RW, UART0 pre-divisor latch byte, only low 7 bit, from 1 to 0/128
#define R8_UART0_ADR \
    (*((PUINT8V)0x4000300F))  // RW, UART0 slave address: 0xFF=disable, other=enable
...
#define R8_UART1_DIV \
    (*((PUINT8V)0x4000340E))  // RW, UART1 pre-divisor latch byte, only low 7 bit, from 1 to 0/128
...
#define R8_UART2_DIV \
    (*((PUINT8V)0x4000380E))  // RW, UART2 pre-divisor latch byte, only low 7 bit, from 1 to 0/128
...
#define R8_UART3_DIV \
    (*((PUINT8V)0x40003C0E))  // RW, UART3 pre-divisor latch byte, only low 7 bit, from 1 to 0/128
...
#define RB_FCR_FIFO_TRIG \
    0xC0  // RW, UART receiver FIFO trigger level: 00-1byte, 01-2bytes, 10-4bytes, 11-7bytes
#define UART_LCR 3
...
#define R32_SPI0_DMA_BEG \
    (*((PUINT32V)0x40004018))  // RW, SPI0 DMA begin address
...
#define RB_SPI_2WIRE_MOD \
    0x04  // RW, SPI0 enable 2 wire mode for slave: 0=3wire(SCK0,MOSI,MISO), 1=2wire(SCK0,MISO=MXSX)
#define RB_SPI_MST_SCK_MOD \
    0x08  // RW, SPI master clock mode: 0=mode 0, 1=mode 3
#define RB_SPI_SLV_CMD_MOD \
    0x08  // RW, SPI0 slave command mode: 0=byte stream, 1=first byte command
#define RB_SPI_FIFO_DIR \
    0x10  // RW, SPI FIFO direction: 0=out(write @master mode), 1=in(read @master mode)
...
#define RB_SPI_AUTO_IF \
    0x10  // RW, enable buffer/FIFO accessing to auto clear RB_SPI_IF_BYTE_END interrupt flag
...
#define RB_SPI_IF_FIFO_HF \
    0x04  // RW1, interrupt flag for SPI FIFO half (RB_SPI_FIFO_DIR ? >=4bytes : <4bytes)
...
#define RB_I2C_START \
    0x0100  // RW, Start generation: master mode: 0=no start, 1=repeated start; slave mode: 0=no start, 1=start at bus free
#define RB_I2C_STOP \
    0x0200  // RW, Stop generation: master mode: 0=no stop, 1=stop after the current byte transfer or after the current Start condition is sent; slave mode: 0=no stop, 1=Release the SCL and SDA lines after the current byte transfer
#define RB_I2C_ACK 0x0400  // RW, Acknowledge enable
#define RB_I2C_POS 0x0800  // RW, Acknowledge/PEC Position (for data reception)
#define RB_I2C_PEC \
    0x1000  // RW, Packet error checking: 0=No PEC transfer, 1=PEC transfer (in Tx or Rx mode)
#define RB_I2C_ALERT \
    0x2000  // RW, SMBus alert: 0=Releases SMBA pin high, 1=Drives SMBA pin low.
#define RB_I2C_SWRST 0x8000  // RW, Software reset
#define I2C_CTRL2    4
#define RB_I2C_FREQ \
    0x003F  // RW, Peripheral clock frequency, The minimum allowed frequency is 2 MHz,the maximum frequency is 36 MHz
...
#define RB_I2C_ADDMODE \
    0x8000  // RW, Addressing mode (slave mode): 0=7-bit slave address, 1=10-bit slave address
...
#define RB_I2C_DUALF \
    0x0080  // RO, Dual flag (Slave mode): 0=Received address matched with OAR1, 1=Received address matched with OAR2
...
#define I2C_RTR 32
#define RB_I2C_TRISE \
    0x003F  // RW, Maximum rise time in Fm/Sm mode (Master mode)
...
#define R16_PWM9_REG_DATA \
    (*((PUINT16V)0x40005012))  // RW, PWM9 16bit data holding
...
#define BA_PWMX \
    ((PUINT8V)0x40005000)  // point PWM4/5/6/7/8/9/10/11 base address
#define PWM_OUT_EN      0
...
#define RB_PWM4_POLAR \
    0x01  // RW, PWM4 output polarity: 0=default low and high action, 1=default high and low action
#define RB_PWM5_POLAR \
    0x02  // RW, PWM5 output polarity: 0=default low and high action, 1=default high and low action
#define RB_PWM6_POLAR \
    0x04  // RW, PWM6 output polarity: 0=default low and high action, 1=default high and low action
#define RB_PWM7_POLAR \
    0x08  // RW, PWM7 output polarity: 0=default low and high action, 1=default high and low action
#define RB_PWM8_POLAR \
    0x10  // RW, PWM8 output polarity: 0=default low and high action, 1=default high and low action
#define RB_PWM9_POLAR \
    0x20  // RW, PWM9 output polarity: 0=default low and high action, 1=default high and low action
#define RB_PWM10_POLAR \
    0x40  // RW, PWM10 output polarity: 0=default low and high action, 1=default high and low action
#define RB_PWM11_POLAR \
    0x80  // RW, PWM11 output polarity: 0=default low and high action, 1=default high and low action
#define PWM_CONFIG 2
#define RB_PWM_CYCLE_SEL \
    0x01  // RW, PWM cycle selection: 0=256/128/64/32 clocks, 1=255/127/63/31 clocks
#define RB_PWM_STAG_ST 0x02  // RO, PWM stagger cycle status
#define RB_PWM_CYC_MOD \
    0x0C  // RW, PWM data width mode: 00=8 bits data, 01=7 bits data, 10=6 bits data, 11=16 bits data
#define RB_PWM4_5_STAG_EN \
    0x10  // RW, PWM4/5 stagger output enable: 0=independent output, 1=stagger output
#define RB_PWM6_7_STAG_EN \
    0x20  // RW, PWM6/7 stagger output enable: 0=independent output, 1=stagger output
#define RB_PWM8_9_STAG_EN \
    0x40  // RW, PWM8/9 stagger output enable: 0=independent output, 1=stagger output
#define RB_PWM10_11_STAG_EN \
    0x80  // RW, PWM10/11 stagger output enable: 0=independent output, 1=stagger output
...
/* LED register */
#define R8_LED_CTRL_MOD \
    (*((PUINT8V)0x4000F000))  // RW, LED mode config register
#define RB_LED_BIT_ORDER \
    0x01  // RW, LED serial bit director select 0=hign first, 1=low first
...
#define RB_LED_CHAN_MOD \
    0xC0  // RW, LED channel mode select, 00=LED0, 01=LED0/1, 10=LED0~3, 11=LED0~7
#define R8_LED_CLOCK_DIV \
    (*((PUINT8V)0x4000F001))  // RW, LED serial clk div register
#define RB_LED_CLOCK_DIV 0xFF  // RW, paramater of LED output clk divider
#define R8_LED_CTRL_MOD1 \
    (*((PUINT8V)0x4000F002))  // RW, LED mode config1 register
...
#define RB_LED_IF_DMA_INT \
    0x0200  // RW1, LED DMA transfer complete flag, clear RB_LED_IF_DMA_INT
...
#define R32_LED_DMA_CUR \
    (*((PUINT32V)0x4000F01C))  // RW, LED DMA current address
...
#define RB_LCD_DUTY \
    0x18  // RW, LCD duty select:  00=1/2 duty,  01=1/3 duty,  10=1/4 duty
#define RB_LCD_SCAN_CLK \
    0x60  // RW, LCD scan clock select: 00=256Hz, 01=512Hz, 10=1KHz, 11=128Hz
#define RB_LCD_VLCD_SEL \
    0x80  // RW, LCD drive voltage select 0=VIO33*100%(3.3V),1=VIO33*76%(2.5V)

#define R32_LCD_RAM0 \
    (*((PUINT32V)(0x40006004)))  // RW, LCD driver data0, address 0-3
#define R32_LCD_RAM1 \
    (*((PUINT32V)(0x40006008)))  // RW, LCD driver data1, address 4-7
#define R32_LCD_RAM2 \
...
#define DATA_FLASH_SIZE_584X \
    0x80000  // size of 584X Data-Flash, only accessible through libISP585
...
#define INT_ADDR_GPIO_A \
    (INT_ID_GPIO_A *    \
     INT_VEC_ENTRY_SZ)  // interrupt vector address for GPIO port A
#define INT_ADDR_GPIO_B \
    (INT_ID_GPIO_B *    \
     INT_VEC_ENTRY_SZ)  // interrupt vector address for GPIO port B
...
#define INT_ADDR_PWMX \
    (INT_ID_PWMX * INT_VEC_ENTRY_SZ)  // interrupt vector address for PWM4~11
...
#define INT_ADDR_WDOG_BAT \
    (INT_ID_WDOG_BAT *    \
     INT_VEC_ENTRY_SZ)  // interrupt vector address for Watch-Dog timer and Battery low voltage
...
#define RB_UC_HOST_MODE \
    0x80  // enable USB host mode: 0=device mode, 1=host mode
...
#define RB_UC_INT_BUSY \
    0x08  // enable automatic responding busy for device mode or automatic pause for host mode during interrupt flag UIF_TRANSFER valid
...
#define RB_UH_PORT_EN \
    0x01  // enable USB port: 0=disable, 1=enable port, automatic disabled if USB device detached
...
#define R32_USB_STATUS \
    (*((PUINT32V)0x40008004))  // USB miscellaneous status & interrupt flag & interrupt status
...
#define RB_UIF_FIFO_OV \
    0x10  // FIFO overflow interrupt flag for USB, direct bit address clear or write 1 to clear
...
#define RB_UIF_TRANSFER \
    0x02  // USB transfer completion interrupt flag, direct bit address clear or write 1 to clear
...
#define RB_UIS_SETUP_ACT \
    0x80  // RO, indicate SETUP token & 8 bytes setup request received for USB device mode
...
#define MASK_UIS_H_RES \
    0x0F  // RO, bit mask of current transfer handshake response for USB host mode: 0000=no response, time out from device, others=handshake response PID received
...
#define RB_UEP_R_TOG \
    0x80  // expected data toggle flag of USB endpoint X receiving (OUT): 0=DATA0, 1=DATA1
#define RB_UEP_T_TOG \
    0x40  // prepared data toggle flag of USB endpoint X transmittal (IN): 0=DATA0, 1=DATA1
#define RB_UEP_AUTO_TOG \
    0x10  // enable automatic toggle after successful transfer completion on endpoint 1/2/3: 0=manual toggle, 1=automatic toggle
...
#define MASK_UEP_T_RES \
    0x03  // bit mask of handshake response type for USB endpoint X transmittal (IN)
...
#define RB_UH_R_RES \
    0x04  // prepared handshake response type for host receiving (IN): 0=ACK (ready), 1=no response, time out to device, for isochronous transactions
...
#define RB_UH_T_TOG \
    0x40  // prepared data toggle flag of host transmittal (SETUP/OUT): 0=DATA0, 1=DATA1
#define RB_UH_T_AUTO_TOG \
    0x10  // enable automatic toggle after successful transfer completion: 0=manual toggle, 1=automatic toggle
#define RB_UH_T_RES \
    0x01  // expected handshake response type for host transmittal (SETUP/OUT): 0=ACK (ready), 1=no response, time out from device, for isochronous transactions
...
#define R8_USB2_CTRL \
    (*((PUINT8V)0x40009000))  // RW, USB_high_speed control register
...
#define USBHS_UD_RST_SIE \
    0x02  // RW, reset USB protocol processor,including end point register
#define USBHS_UD_RST_LINK 0x01  // RW, enable LNK layer reset
#define R8_USB2_BASE_MODE \
    (*((PUINT8V)0x40009001))  // RW, USB_high_speed mode control register
#define USBHS_UD_SPEED_FULL 0x00
#define USBHS_UD_SPEED_HIGH 0x01
#define USBHS_UD_SPEED_LOW  0x02
#define USBHS_UD_SPEED_TYPE \
    0x03  // RW, speed mode excpeted by the equipment,00:full speed, 01:high speed, 10:low speed
#define R8_USB2_INT_EN \
    (*((PUINT8V)0x40009002))  // RW, USB_high_speed intreurpt enable register
...
#define R8_USB2_DEV_AD \
    (*((PUINT8V)0x40009003))  // RW, USB_high_speed device adress register
#define USBHS_UD_DEV_ADDR 0x7F  // RW, adress of usb equipment
#define R8_USB2_WAKE_CTRL \
    (*((PUINT8V)0x40009004))  // RW, USB_high_speed wake up remotely register
#define USBHS_UD_UD_REMOTE_WKUP \
    0x01  // RW1, wake up remotely and auto reset hardware
#define R8_USB2_TEST_MODE \
    (*((PUINT8V)0x40009005))  // RW, USB_high_speed test mode register
#define USBHS_UD_TEST_EN     0x80  // RW, enable test mode
#define USBHS_UD_TEST_SE0NAK 0x08  // RW, output SE0 when in test mode
#define USBHS_UD_TEST_PKT \
    0x04  // RW, output one package(including DATA0,data and length of end pont4) when in test mode,not work on virtual equipment
#define USBHS_UD_TEST_K 0x02  // RW, output K when in test mode
#define USBHS_UD_TEST_J 0x01  // RW, output J when in test mode
#define R16_USB2_LPM_DATA \
    (*((PUINT16V)0x40009006))  // RW, USB_high_speed power control register
#define USBHS_UD_LPM_BUSY 0x8000  // RW, power control busy
#define USBHS_UD_LPM_DATA 0x07FF  // RO, power control data
#define R8_USB2_INT_FG \
...
#define R8_USB2_INT_ST \
    (*((PUINT8V)0x40009009))  // RW, USB_high_speed interupt status register
#define USBHS_UDIS_EP_DIR 0x10  // RO, end point tranfer diector of data
#define USBHS_UDIS_EP_ID_MASK \
    0x07  // RO, number of end point which data transmission occured
#define R8_USB2_MIS_ST \
    (*((PUINT8V)0x4000900A))  // RW, USB_high_speed miscellaneous register
...
#define R16_USB2_FRAME_NO \
    (*((PUINT16V)0x4000900C))  // RW, USB_high_speed frame number register
#define USBHS_UD_MFRAME_NO 0xE000
#define USBHS_UD_FRAME_NO  0x07FF
#define R16_USB2_BUS \
    (*((PUINT16V)0x4000900E))  // RW, USB_high_speed bus status register
#define USBHS_USB_DM_ST 0x08
#define USBHS_USB_DP_ST 0x04
#define USB_WAKEUP      0x01
#define R16_U2EP_TX_EN \
    (*((PUINT16V)0x40009010))  // RW, USB_high_speed end point transmit enable register
...
#define R16_U2EP_RX_EN \
    (*((PUINT16V)0x40009012))  // RW, USB_high_speed end point receive enableregister
#define USBHS_UEP_RX_EN 0xFFFF
#define R16_U2EP_T_TOG_AUTO \
    (*((PUINT16V)0x40009014))  // RW, USB_high_speed end point transmit auto toggle enable register
#define USBHS_UEP_T_TOG_AUTO 0xFF
#define R16_U2EP_R_TOG_AUTO \
    (*((PUINT16V)0x40009016))  // RW, USB_high_speed end point receive auto toggle enable register
#define USBHS_UEP_R_TOG_AUTO 0xFF
#define R8_U2EP_T_BURST \
    (*((PUINT8V)0x40009018))  // RW, USB_high_speed end point transmit burst register
#define USBHS_UEP_T_BURST_EN 0xFF
#define R8_U2EP_T_BURST_MODE \
    (*((PUINT8V)0x40009019))  // RW, USB_high_speed end point transmit burst mode register
#define USBHS_UEP_T_BURST_MODE 0xFF
#define R8_U2EP_R_BURST \
    (*((PUINT8V)0x4000901A))  // RW, USB_high_speed end point receive burst register
#define USBHS_UEP_R_BURST_EN 0xFF
#define R8_U2EP_R_RES_MODE \
    (*((PUINT8V)0x4000901B))  // RW, USB_high_speed end point transmit reply mode register
#define USBHS_UEP_R_RES_MODE 0xFF
#define R32_U2EP_AF_MODE \
    (*((PUINT32V)0x4000901C))  // RW, USB_high_speed end point multiplexing register
#define USBHS_UEP_T_AF 0xFE
#define R32_U2EP0_DMA \
    (*((PUINT32V)0x40009020))  // RW, USB_high_speed end point0 begin adress of DMA buffer register
#define UEPn_DMA 0x01FFFF
#define R32_U2EP1_RX_DMA \
    (*((PUINT32V)0x40009024))  // RW, USB_high_speed end point1 begin adress of DMA receive buffer register
#define R32_U2EP2_RX_DMA \
    (*((PUINT32V)0x40009028))  // RW, USB_high_speed end point2 begin adress of DMA receive buffer register
#define R32_U2EP3_RX_DMA \
    (*((PUINT32V)0x4000902C))  // RW, USB_high_speed end point3 begin adress of DMA receive buffer register
#define R32_U2EP4_RX_DMA \
    (*((PUINT32V)0x40009030))  // RW, USB_high_speed end point4 begin adress of DMA receive buffer register
#define R32_U2EP5_RX_DMA \
    (*((PUINT32V)0x40009034))  // RW, USB_high_speed end point5 begin adress of DMA receive buffer register
#define R32_U2EP6_RX_DMA \
    (*((PUINT32V)0x40009038))  // RW, USB_high_speed end point6 begin adress of DMA receive buffer register
#define R32_U2EP7_RX_DMA \
    (*((PUINT32V)0x4000903C))  // RW, USB_high_speed end point7 begin adress of DMA receive buffer register
#define UEPn_RX_DMA 0x01FFFF
#define R32_U2EP1_TX_DMA \
    (*((PUINT32V)0x40009040))  // RW, USB_high_speed end point1 begin adress of DMA transmit buffer register
#define R32_U2EP2_TX_DMA \
    (*((PUINT32V)0x40009044))  // RW, USB_high_speed end point2 begin adress of DMA transmit buffer register
#define R32_U2EP3_TX_DMA \
    (*((PUINT32V)0x40009048))  // RW, USB_high_speed end point3 begin adress of DMA transmit buffer register
#define R32_U2EP4_TX_DMA \
    (*((PUINT32V)0x4000904C))  // RW, USB_high_speed end point4 begin adress of DMA transmit buffer register
#define R32_U2EP5_TX_DMA \
    (*((PUINT32V)0x40009050))  // RW, USB_high_speed end point5 begin adress of DMA transmit buffer register
#define R32_U2EP6_TX_DMA \
    (*((PUINT32V)0x40009054))  // RW, USB_high_speed end point6 begin adress of DMA transmit buffer register
#define R32_U2EP7_TX_DMA \
    (*((PUINT32V)0x40009058))  // RW, USB_high_speed end point7 begin adress of DMA transmit buffer register
#define UEPn_TX_DMA 0x01FFFF
#define R32_U2EP0_MAX_LEN \
    (*((PUINT32V)0x4000905C))  // RW, USB_high_speed end point0 max length package register
#define R32_U2EP1_MAX_LEN \
    (*((PUINT32V)0x40009060))  // RW, USB_high_speed end point1 max length package register
#define R32_U2EP2_MAX_LEN \
    (*((PUINT32V)0x40009064))  // RW, USB_high_speed end point2 max length package register
#define R32_U2EP3_MAX_LEN \
    (*((PUINT32V)0x40009068))  // RW, USB_high_speed end point3 max length package register
#define R32_U2EP4_MAX_LEN \
    (*((PUINT32V)0x4000906C))  // RW, USB_high_speed end point4 max length package register
#define R32_U2EP5_MAX_LEN \
    (*((PUINT32V)0x40009070))  // RW, USB_high_speed end point5 max length package register
#define R32_U2EP6_MAX_LEN \
    (*((PUINT32V)0x40009074))  // RW, USB_high_speed end point6 max length package register
#define R32_U2EP7_MAX_LEN \
    (*((PUINT32V)0x40009078))  // RW, USB_high_speed end point7 max length package register
#define UEPn_MAX_LEN 0x007F
#define R16_U2EP0_RX_LEN \
    (*((PUINT16V)0x4000907C))  // RW, USB_high_speed end point0 length of receive register
#define UEP0_RX_LEN 0x007F
#define R16_U2EP1_RX_LEN \
    (*((PUINT16V)0x40009080))  // RW, USB_high_speed end point1 single received length register
#define R16_U2EP1_R_SIZE \
    (*((PUINT16V)0x40009082))  // RW, USB_high_speed end point1 total received length register
#define R16_U2EP2_RX_LEN \
    (*((PUINT16V)0x40009084))  // RW, USB_high_speed end point2 single received length register
#define R16_U2EP2_R_SIZE \
    (*((PUINT16V)0x40009086))  // RW, USB_high_speed end point2 total received length register
#define R16_U2EP3_RX_LEN \
    (*((PUINT16V)0x40009088))  // RW, USB_high_speed end point3 single received length register
#define R16_U2EP3_R_SIZE \
    (*((PUINT16V)0x4000908A))  // RW, USB_high_speed end point3 total received length register
#define R16_U2EP4_RX_LEN \
    (*((PUINT16V)0x4000908C))  // RW, USB_high_speed end point4 single received length register
#define R16_U2EP4_R_SIZE \
    (*((PUINT16V)0x4000908E))  // RW, USB_high_speed end point4 total received length register
#define R16_U2EP5_RX_LEN \
    (*((PUINT16V)0x40009090))  // RW, USB_high_speed end point5 single received length register
#define R16_U2EP5_R_SIZE \
    (*((PUINT16V)0x40009092))  // RW, USB_high_speed end point5 total received length register
#define R16_U2EP6_RX_LEN \
    (*((PUINT16V)0x40009094))  // RW, USB_high_speed end point6 single received length register
#define R16_U2EP6_R_SIZE \
    (*((PUINT16V)0x40009096))  // RW, USB_high_speed end point6 total received length register
#define R16_U2EP7_RX_LEN \
    (*((PUINT16V)0x40009098))  // RW, USB_high_speed end point7 single received length register
#define R16_U2EP7_R_SIZE \
    (*((PUINT16V)0x4000909A))  // RW, USB_high_speed end point7 total received length register
#define UEPn_RX_LEN 0xFFFF
#define UEPn_R_SIZE 0xFFFF
#define R16_U2EP0_T_LEN \
    (*((PUINT16V)0x4000909C))  // RW, USB_high_speed end point0 length of transmission register
#define UEP0_T_LEN 0x7F
#define R8_U2EP0_TX_CTRL \
    (*((PUINT8V)0x4000909E))  // RW, USB_high_speed end point0 transmit control register
#define R8_U2EP0_RX_CTRL \
    (*((PUINT8V)0x4000909F))  // RW, USB_high_speed end point0 receive control register
#define R16_U2EP1_T_LEN \
    (*((PUINT16V)0x400090A0))  // RW, USB_high_speed end point1 length of transmission register
#define R8_U2EP1_TX_CTRL \
    (*((PUINT8V)0x400090A2))  // RW, USB_high_speed end point1 transmit control register
#define R8_U2EP1_RX_CTRL \
    (*((PUINT8V)0x400090A3))  // RW, USB_high_speed end point1 receive control register
#define R16_U2EP2_T_LEN \
    (*((PUINT16V)0x400090A4))  // RW, USB_high_speed end point2 length of transmission register
#define R8_U2EP2_TX_CTRL \
    (*((PUINT8V)0x400090A6))  // RW, USB_high_speed end point2 transmit control register
#define R8_U2EP2_RX_CTRL \
    (*((PUINT8V)0x400090A7))  // RW, USB_high_speed end point2 receive control register
#define R16_U2EP3_T_LEN \
    (*((PUINT16V)0x400090A8))  // RW, USB_high_speed end point3 length of transmission register
#define R8_U2EP3_TX_CTRL \
    (*((PUINT8V)0x400090AA))  // RW, USB_high_speed end point3 transmit control register
#define R8_U2EP3_RX_CTRL \
    (*((PUINT8V)0x400090AB))  // RW, USB_high_speed end point3 receive control register
#define R16_U2EP4_T_LEN \
    (*((PUINT16V)0x400090AC))  // RW, USB_high_speed end point4 length of transmission register
#define R8_U2EP4_TX_CTRL \
    (*((PUINT8V)0x400090AE))  // RW, USB_high_speed end point4 transmit control register
#define R8_U2EP4_RX_CTRL \
    (*((PUINT8V)0x400090AF))  // RW, USB_high_speed end point4 receive control register
#define R16_U2EP5_T_LEN \
    (*((PUINT16V)0x400090B0))  // RW, USB_high_speed end point5 length of transmission register
#define R8_U2EP5_TX_CTRL \
    (*((PUINT8V)0x400090B2))  // RW, USB_high_speed end point5 transmit control register
#define R8_U2EP5_RX_CTRL \
    (*((PUINT8V)0x400090B3))  // RW, USB_high_speed end point5 receive control register
#define R16_U2EP6_T_LEN \
    (*((PUINT16V)0x400090B4))  // RW, USB_high_speed end point6 length of transmission register
#define R8_U2EP6_TX_CTRL \
    (*((PUINT8V)0x400090B6))  // RW, USB_high_speed end point6 transmit control register
#define R8_U2EP6_RX_CTRL \
    (*((PUINT8V)0x400090B7))  // RW, USB_high_speed end point6 receive control register
#define R16_U2EP7_T_LEN \
    (*((PUINT16V)0x400090B8))  // RW, USB_high_speed end point7 length of transmission register
#define R8_U2EP7_TX_CTRL \
    (*((PUINT8V)0x400090BA))  // RW, USB_high_speed end point7 transmit control register
#define R8_U2EP7_RX_CTRL \
    (*((PUINT8V)0x400090BB))  // RW, USB_high_speed end point7 receive control register
/**R16_UEPn_T_LEN**/
...
#define R16_U2EP_T_ISO \
    (*((PUINT16V)0x400090BC))  // RW, USB_high_speed end point transmit sync mode register
...
#define R8_U2H_PORT_TEST_CT \
    (*((PUINT8V)0x40009133))  // RW, USB_high_speed register
#define USBHS_UH_TEST_FORCE_EN 0x04
#define USBHS_UH_TEST_K        0x02
#define USBHS_UH_TEST_J        0x01
#define R16_U2H_PORT_ST \
    (*((PUINT16V)0x40009134))  // RW, USB_high_speed register
