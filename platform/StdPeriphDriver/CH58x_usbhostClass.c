...
/* 设置HID上传速率 */
__attribute__((aligned(4))) const uint8_t SetupSetHIDIdle[] = {
...
const uint8_t SetupGetHubDescr[] = {HUB_GET_HUB_DESCRIPTOR,
                                    HUB_GET_DESCRIPTOR,
                                    0x00,
                                    USB_DESCR_TYP_HUB,
                                    0x00,
...
__attribute__((aligned(4))) uint8_t Com_Buffer
    [128];  // 定义用户临时缓冲区,枚举时用于处理描述符,枚举结束也可以用作普通临时缓冲区
...
uint8_t AnalyzeHidIntEndp(uint8_t *buf, uint8_t HubPortIndex) {
    ...
    for (i = 0; i < ((PUSB_CFG_DESCR)buf)->wTotalLength;
...
uint8_t InitRootDevice(void) {
    ...
    if (s == ERR_SUCCESS) {
        ...
        ThisUsbDev.DeviceVID =
            ((PUSB_DEV_DESCR)Com_Buffer)->idVendor;  //保存VID PID信息
        ...
        if (s == ERR_SUCCESS) {
            ...
            if (s == ERR_SUCCESS) {
                ...
                if_cls = ((PUSB_CFG_DESCR_LONG)Com_Buffer)
                ...
                if ((dv_cls == 0x00) &&
                    ...
                    if (s == ERR_SUCCESS) {
                        ...
                        return (ERR_SUCCESS);
                           ...
                           ((PUSB_CFG_DESCR_LONG)Com_Buffer)
                    ...
                    if (s == ERR_SUCCESS) {
                        ...
                        s = ERR_USB_UNSUPPORT;
                           ...
                           USB_DEV_CLASS_HUB) {  // 是HUB类设备,集线器等
                    s = CtrlGetHubDescr();
                    if (s == ERR_SUCCESS) {
                        PRINT("Max Port:%02X ",
