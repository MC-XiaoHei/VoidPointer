...
const uint8_t SetupGetCfgDescr[] = {USB_REQ_TYP_IN, USB_GET_DESCRIPTOR,
...
__attribute__((aligned(4))) const uint8_t SetupSetUsbAddr[] = {USB_REQ_TYP_OUT,
                                                               ...
                                                               0x00};
/*设置USB配置*/
__attribute__((aligned(4))) const uint8_t SetupSetUsbConfig[] = {
    USB_REQ_TYP_OUT, USB_SET_CONFIGURATION, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
/*设置USB接口配置*/
__attribute__((aligned(4)))
const uint8_t SetupSetUsbInterface[] = {USB_REQ_RECIP_INTERF,
                                        USB_SET_INTERFACE,
                                        0x00,
                                        0x00,
                                        0x00,
...
/*清除端点STALL*/
__attribute__((aligned(4)))
const uint8_t SetupClrEndpStall[] = {USB_REQ_TYP_OUT | USB_REQ_RECIP_ENDP,
                                     USB_CLEAR_FEATURE,
                                     0x00,
                                     0x00,
                                     0x00,
...
void ResetRootHubPort(void) {
    ...
    SetUsbSpeed(1);  // 默认为全速
    R8_UHOST_CTRL = (R8_UHOST_CTRL & ~RB_UH_LOW_SPEED) |
                    RB_UH_BUS_RESET;  // 默认为全速,开始复位
...
uint8_t HostCtrlTransfer(uint8_t *DataBuf, uint8_t *RetLen) {
    ...
    s = USBHostTransact(USB_PID_SETUP << 4 | 0x00, 0x00,
                        200000 / 20);  // SETUP阶段,200mS超时
    if (s != ERR_SUCCESS) {
        return (s);
    }
    R8_UH_RX_CTRL = R8_UH_TX_CTRL = RB_UH_R_TOG | RB_UH_R_AUTO_TOG |
                                    RB_UH_T_TOG |
    ...
    if (RemLen && pBuf)  // 需要收发数据
    {
        ...
        if (pSetupReq->bRequestType & USB_REQ_TYP_IN)  // 收
        {
            while (RemLen) {
                mDelayuS(200);
                s = USBHostTransact(USB_PID_IN << 4 | 0x00, R8_UH_RX_CTRL,
                                    200000 / 20);  // IN数据
                if (s != ERR_SUCCESS) {
                    return (s);
                }
                ...
                if (R8_USB_RX_LEN == 0 ||
                    (R8_USB_RX_LEN & (UsbDevEndp0Size - 1))) {
                    break;  // 短包
                }
            }
        ...
        } else  // 发
        {
            while (RemLen) {
                ...
                s = USBHostTransact(USB_PID_OUT << 4 | 0x00, R8_UH_TX_CTRL,
                                    200000 / 20);  // OUT数据
                if (s != ERR_SUCCESS) {
                    return (s);
                }
