...
void SetSysClock(SYS_CLKTypeDef sc) {
    ...
    if (sc == RB_CLK_SYS_MOD)  // 32KHz
    {
        ...
        if (sc & RB_OSC32M_SEL) {
            if ((!((R8_GLOB_CFG_INFO & RB_CFG_DEBUG_EN) |
                   (R8_GLOB_CFG_INFO & RB_CFG_ROM_READ))) &&
 ...
 * @return  null
 */
void SYS_EnablePI() { MachineMode_Call(SetPI_func); }
...
uint32_t GetSysClock(void) {
    if ((R16_CLK_SYS_CFG & RB_CLK_SYS_MOD) == RB_CLK_SYS_MOD) {  // 32K做主频
        return (CAB_LSIFQ);
    ...
    } else if (R16_CLK_SYS_CFG & RB_OSC32M_SEL) {  // 32M进行分频
...
#if ((FREQ_SYS == 78000000) || (FREQ_SYS == 39000000) || (FREQ_SYS == 19500000))
    while (--i) {
        __nop();
    };
#else
 ...
 * @return  none
 */
void IWDG_KR_Set(IWDG_KR_Key kr) { R32_IWDG_KR = kr; }
...
uint8_t IWDG_Enable(IWDG_32K_PR pr, uint16_t rlr) {
    ...
    sys_safe_access_enable();
    R8_SAFE_LRST_CTRL |=
        RB_IWDG_RST_EN;  //看门狗复位为上电复位。若改为全局复位，需保证LSI不被关闭
    sys_safe_access_disable();
