...
- [ ] 补齐 `prepare/enter sleep` 平台 API。
- [ ] 补齐 wake source 配置和 resume 后外设恢复。
- [ ] 将 power profile 切换和 IMU profile 切换真正连上。
...
- [x] power timeout 使用 delayed poll 重新评估。
- [x] Power transition 已走 RuntimeCommand 边界。

### 未完成
- [ ] 平台 `prepare/enter suspend`。
- [ ] 平台 `prepare/enter sleep`。
- [ ] wake source 配置。
- [ ] wake 后外设恢复。
