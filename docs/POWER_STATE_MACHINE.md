...
> 当前实现说明：`c_vp_power_prepare_sleep()` / `enter_sleep()` / `restore_from_sleep()` 已补成项目级最小路径：进入前关闭 Laser、切 IMU sleep profile、关闭 BLE advertising；恢复时仅在 **USB 未 configured** 时重新打开 BLE advertising，再交由 Rust 恢复 Active IMU profile 与后续状态评估。恢复回 `Active` 时，runtime 还会清掉旧 attitude / IMU sample / motion cache，并重置 report 累积状态，避免 wake 后第一帧 motion 误吃到低功耗前的陈旧姿态或残余累计量。它仍不是 CH585 真实 deep-sleep 映射。
