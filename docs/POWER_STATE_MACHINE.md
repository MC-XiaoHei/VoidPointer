...
- `Active` / `Suspend` / `Sleep` 三态都进入实现，不把 `Sleep` 整体留作 stub。
- `Sleep` 的参数、IMU profile 和 CH585 power plan 允许后续按功耗实测调优。
- 因此 v1 需要具备完整的状态转换、blocker 判断、wake source 配置入口和 wake 后恢复路径；实测数值不足时在 profile 参数上标记 tuning required。
...
| --- | --- | --- | --- |
| BLE / 2.4G RF | 按 route 策略启用 | 保持已连接 route | 关闭或不保持连接 |
| USB | 正常处理 attach/configured/vendor | USB configured 时不进入 `Suspend`；USB detached 场景按无线/低功耗策略处理 | USB attached/configured 时禁止进入；仅 USB detached 时允许 |
| IMU | Active profile，SFLP/FIFO 用于 motion | Suspend profile，低功耗 motion/activity wake | Sleep profile，SFLP off，gyro power-down，accel wake |
...
| 唤醒源 | 全部事件 | GPIO/encoder/IMU/USB/route event | GPIO/IMU/USB attach/route event，具体待确认 |
...
Vendor/WebHID config 会话不作为特殊 blocker；如果满足普通 idle 条件，允许进入 `Suspend`。
