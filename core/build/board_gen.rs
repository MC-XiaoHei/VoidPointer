...
fn gen_board_map_default(defs: &[Def]) -> String {
    let mut s = String::new();
...
fn gen_board_signal_get() -> &'static str {
...
fn gen_board_signal_is_present() -> &'static str {
...
fn gen_gpio_init(defs: &[Def]) -> String {
    let mut s = String::new();
    s.push_str("void board_gpio_init_all(void) {\n");
...
pub fn generate_c_source(defs: &[Def]) -> String {
    ...
    s.push_str(&gen_board_map_default(defs));
    s.push_str(&gen_board_signal_get());
    s.push_str(&gen_board_signal_is_present());
    s.push_str(&gen_board_remap_reset());
    s.push_str(&gen_board_remap_apply());
    s.push_str(&gen_board_signal_get_channel(defs));

    s.push_str(&gen_polarity_array(defs));
    s.push_str("uint8_t board_signal_get_polarity(BoardSignal sig) {\n");
    s.push_str("    if (sig >= BOARD_SIGNAL_COUNT) { return 0u; }\n");
    s.push_str("    return BOARD_SIGNAL_POLARITY[sig];\n");
    s.push_str("}\n\n");

    check_remap_conflicts(defs);
    s.push_str(&gen_gpio_init(defs));
    s
}
